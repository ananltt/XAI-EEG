...
@credits: Alberto Zancanaro (Jesus)
"""
...
class FBCSP():
    ...
    def __init__(self, data_dict, fs, n_w=2, n_features=4, freqs_band=None, filter_order=3, classifier=None,
                 print_var=True):
        self.fs = fs
    ...
    def filterBankFunction(self, filter_order=3):
        ...
        for i in range(len(self.freqs) - 1):
            ...
            for key in self.trials_dict.keys():
                ...
                filt_trial_dict[key] = self.bandFilterTrials(self.trials_dict[key], band[0], band[1],
    ...
    def changeShapeMutualInformationList(self):
        ...
        for i in range(len(self.mutual_information_list)):
            ...
            for j in range(self.n_w * 2):
                ...
                other_info_matrix[actual_idx, 0] = i * self.n_w * 2 + (
                            (self.n_w * 2) - (j + 1))  # Position of the twin (in the vector)
